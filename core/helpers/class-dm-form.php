...
class DM_Form {
	 ...
	 * @var DM_Form[] {'form_id' => instance}
	 *
	 * @deprecated 2.6.15 Use DM_Form::get_forms()
	 */
	 ...
	 */
	protected static $id_input_name = 'f';
	...
	public function __construct( $id, $data = array() ) {
		try {
			self::get_form( $id );
			trigger_error( sprintf( __( 'Form with id "%s" was already defined', '' ), $id ), E_USER_ERROR );

			return;
		} catch ( DM_Form_Not_Found_Exception $e ) {
		}
		...
		$this
			...
			// prepare callbacks
			->set_callbacks( array(
				'render'   => dm_akg( 'render', $data, false ),
				'validate' => dm_akg( 'validate', $data, false ),
				'save'     => dm_akg( 'save', $data, false ),
			) )
			// prepare attributes
			->set_attr( (array) dm_akg( 'attr', $data, array() ) );
		...
		add_action( 'DM_Form_display:before_form', array( $this, '_action_DM_Form_show_errors' ) );
	}
	...
	public function _validate_and_save() {
		...
		try {
			...
			if ( ( $redirect = dm_akg( 'redirect', $data ) ) ) {
				wp_redirect( $redirect );
				exit;
			}
		} catch ( DM_Form_Invalid_Submission_Exception $e ) {
			if ( $this->_is_ajax() ) {
	 ...
	 */
	public function _action_DM_Form_show_errors( $form ) {
		if (
		 ...
		 */
		do_action( 'DM_Form_display_errors_frontend', $form );
	...
	public function render( $data = array() ) {
		...
		if ( $render_callback = dm_akg( 'render', $this->get_callbacks() ) ) {
			ob_start();
		...
		do_action( 'DM_Form_display:before_form', $this );

		// display form errors in frontend
		echo '<form ' . dm_attr_to_html( $render_data['attr'] ) . ' >';

		do_action( 'DM_Form_display:before', $this );

		echo dm_html_tag( 'input',
			array(
				'type'  => 'hidden',
		...
		if ( isset( $render_data['submit']['html'] ) ) {
			echo render($render_data['submit']['html']);
		} else {
			echo dm_html_tag( 'input',
				array(
					'type'  => 'submit',
		...
		do_action( 'DM_Form_display:after', $this );

		echo '</form>';

		do_action( 'DM_Form_display:after_form', $this );
	}
	 ...
	 * @param array $request
	 *
	 * @throws DM_Form_Invalid_Submission_Exception
	 *
	 * @return mixed
	 */
	public function submit( array $request = array() ) {
		...
		if ( ! empty( $errors ) ) {
			throw new DM_Form_Invalid_Submission_Exception( $errors );
		}
	...
	protected function get_default_attr() {
		return array(
			'data-dm-form-id' => $this->get_id(),
			'method'          => 'post',
			'action'          => dm_current_url(),
			'class'           => 'DM_Form_' . $this->get_id()
		);
	}
	...
	protected function get_request( $key = null ) {
		return $key === null ? (array) $this->request : dm_akg( $key, $this->request );
	}
	...
	protected function validate() {
		...
		if ( ! $this->check_nonce( $this->get_nonce() ) ) {
			$errors[ $this->get_nonce_name() ] = esc_html__( 'Your session expired. Please refresh page and try again.', 'dm' );
		}
		 ...
		 */
		if ( ( $validate = dm_akg( 'validate', $this->get_callbacks() ) ) ) {
	...
	protected function save() {
		 ...
		 * Callback must 'manually' extract input values from $_POST (or $_GET)
		 */
		if ( ( $save_callback = dm_akg( 'save', $this->get_callbacks() ) ) ) {
	...
	private function get_nonce_name( $render_data = array() ) {
		return '_nonce_' . md5( $this->id . apply_filters( 'dm:form:nonce-name-data', '', $this, $render_data ) );
	}
	 ...
	 *
	 * @return DM_Form
	 * @throws DM_Form_Not_Found_Exception
	 *
	 * @since 2.6.15
	 */
	public static function get_form( $id ) {
		if ( ! isset( self::$forms[ $id ] ) ) {
			throw new DM_Form_Not_Found_Exception( "DM_Form $id was not defined" );
		}
	...
	public static function _is_ajax() {
		return ( defined( 'DOING_AJAX' ) && DOING_AJAX )
		       ||
		       strtolower( dm_akg( 'HTTP_X_REQUESTED_WITH', $_SERVER ) ) == 'xmlhttprequest';
	}

	public static function get_form_request( $id ) {
		if ( DM_Request::POST( self::get_form_id_name() ) == $id ) {
			return DM_Request::POST();
		}

		if ( DM_Request::GET( self::get_form_id_name() ) == $id ) {
			return DM_Request::GET();
		}
	...
	private static function collect_flash_messages() {
		...
		foreach ( DM_Flash_Messages::_get_messages( true ) as $type => $messages ) {
