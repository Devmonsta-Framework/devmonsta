jQuery(document).ready(function($){
	var helpers = {
		optionClass: 'dm-option-type-color-picker',
		eventNamespace: '.dmOptionTypeColorPicker',
		colorRegex: /^#([a-f0-9]{3}){1,2}$/i,
		localized: window._dm_option_type_color_picker_localized,
		/**
	...
	dmEvents.on('dm:options:init', function (data) {
		data.$elements.find('input.'+ helpers.optionClass +':not(.initialized)').each(function(){
			...
			$input.one('focus', function(){
				...
				$input.iris({
					...
					change: function(event, ui){
						...
						changeTimeoutId = setTimeout(function(){
							...
							$input.trigger('dm:color:picker:changed', { // should be 'dm:option-type:color-picker:change'
								$element: $input,
								event   : event,
								ui      : ui
							});
				 ...
				 */
				{
					$input.parent().attr('id', 'dm-color-picker-r-'+ (++helpers.increment));
				...
				$.each([{
					...
					if (data.color && helpers.isColorValid(data.color)) {
						$picker.find('> .iris-picker-inner').append(''
							+ '<div class="' + helpers.optionClass + '-reset-default dm-pull-left">'
