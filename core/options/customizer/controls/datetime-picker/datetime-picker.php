...
class DatetimePicker extends \WP_Customize_Control {
    ...
    public function prepare_values( $id, $args = [] ) {
        ...
        $this->date_time_picker_config = isset($args[0]['datetime-picker']) ? $args[0]['datetime-picker'] : [];
    }
    ...
    public function enqueue() {
        ...
        wp_enqueue_script( 'dm-date-time-picker', DM_CORE . 'options/customizer/controls/datetime-picker/assets/js/script.js', ['jquery', 'flatpickr'], false, true );
        ...
        $date_time_picker_data['minDate']     = isset( $this->date_time_picker_config['min-date'] ) ? date( "Y-m-d H:i", strtotime( $this->date_time_picker_config['min-date'] ) ) : "today";
        $date_time_picker_data['maxDate']     = isset( $this->date_time_picker_config['max-date'] ) ? date( "Y-m-d H:i", strtotime( $this->date_time_picker_config['max-date'] ) ) : false;
        $date_time_picker_data['timepicker']  = ( $this->date_time_picker_config['timepicker'] ) ? 1 : 0;
        $date_time_picker_data['defaultTime'] = isset( $this->date_time_picker_config['defaultTime'] ) ? $this->date_time_picker_config['defaultTime'] : '12:00';
