 ...
 * Rows with options
 */
class DMS_Option_Type_Switch extends DMS_Option_Type
{
	...
	protected function _enqueue_static($id, $option, $data)
	{
		...
		{
			wp_enqueue_style(
				'dms-option-'. $this->get_type() .'-adaptive-switch',
				dms_get_framework_directory_uri('/includes/option-types/'. $this->get_type() .'/static/adaptive-switch/styles.css'),
				array(),
			...
			wp_enqueue_script(
				'dms-option-'. $this->get_type() .'-adaptive-switch',
				dms_get_framework_directory_uri('/includes/option-types/'. $this->get_type() .'/static/adaptive-switch/jquery.adaptive-switch.js'),
				array('jquery'),
				dms()->manifest->get_version(),
				true
			);
		...
		wp_enqueue_style(
			...
			array('dms-option-'. $this->get_type() .'-adaptive-switch'),
			dms()->manifest->get_version()
		);
		...
		wp_enqueue_script(
			...
			array('dms-events', 'dms-option-'. $this->get_type() .'-adaptive-switch'),
			dms()->manifest->get_version(),
			true
		);
	}
	...
	protected function _render($id, $option, $data)
	{
		...
		return '<div '. dms_attr_to_html($option['attr']) .'>'.
			...
			($checked ? '' : dms_html_tag('input', array(
				'type' => 'hidden',
				'name' => $input_attr['name'],
				'value' => $input_attr['data-switch-left-value-json'],
			))).
			'<input type="checkbox" '. dms_attr_to_html($input_attr) .' />'.
		'</div>';
	}
