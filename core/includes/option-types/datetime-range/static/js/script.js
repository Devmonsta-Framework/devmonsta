	...
	$(document).ready(function(){
		var init = function(){
			var $dateTimeRange = $(this),
				$dateTimeFirstWrapper = $dateTimeRange.find('.dms-option-type-datetime-picker:first'),
				$dateTimeLastWrapper = $dateTimeRange.find('.dms-option-type-datetime-picker:last'),
				$dateTimeFirstInput = $dateTimeFirstWrapper.find('input'),
				$dateTimeLastInput = $dateTimeLastWrapper.find('input'),
				dateTimeFirstPicker = $dateTimeFirstInput.data('dms_xdsoft_datetimepicker'),
				dateTimeLastPicker = $dateTimeLastInput.data('dms_xdsoft_datetimepicker');

			dmse.trigger('dms:options:datetime-range:before-init', {el: $dateTimeRange} );
			...
			dateTimeFirstPicker.on('open.dms_xdsoft', function(e){
				...
				//dmse.trigger('dms:datetime-range:first:open', { dateTimePicker: dateTimeFirstPicker, dateTimeInput: $dateTimeFirstInput }); ????
			});
			...
			dateTimeLastPicker.on('open.xdsoft', function(e){
				...
				//dmse.trigger('dms:datetime-range:last:open', { dateTimePicker: dateTimeLastPicker, dateTimeInput: $dateTimeLastInput }); ????
			});
		}

		dmse.on('dms:options:init', function(data) {
			data.$elements
				.find('.dms-option-type-datetime-range:not(.dms-option-initialized)').each(init)
				.addClass('dms-option-initialized');
		});

		dms.options.register('datetime-range', {
		...
		function triggerChangeFor ($container) {
			$container = $($container).closest(
				'[data-dms-option-type="datetime-range"]'
			);

			dms.options.trigger.changeForEl($container, {
		...
		function getValueForEl (el) {
			return {
				from: $(el).find(
					'[data-dms-option-id="from"] input'
				).val(),

				to: $(el).find(
					'[data-dms-option-id="to"] input'
				).val()
			}
		}
