...
class DMS_Option_Type_Datetime_Range extends DMS_Option_Type {

	private function  _get_static_uri() {
		return dms_get_framework_directory_uri('/includes/option-types/datetime-range/static');
	}
	...
	{
		wp_enqueue_style('dms-option-datetime-range-CSS', $this->_get_static_uri() . '/css/styles.css' );
		wp_enqueue_script( 'dms-option-datetime-range-js', $this->_get_static_uri() . '/js/script.js', array('jquery', 'dms-events'));

		dms()->backend->option_type('datetime-picker')->enqueue_static();
	}
	...
	protected function _render( $id, $option, $data ) {
		...
		return dms_render_view( dirname(__FILE__) . '/view.php', array(
			'id' => $id,
			'option' => $option,
			'data' => $data
		));
	...
	protected function _get_value_from_input($option, $input_value)
	{
		...
		$from = dms()->backend->option_type('datetime-picker')->get_value_from_input(array('datetime-picker' => $option['datetime-pickers']['from'], 'value' => $option['value']['from'] ), $input_value['from']);
		$to = dms()->backend->option_type('datetime-picker')->get_value_from_input(array('datetime-picker' => $option['datetime-pickers']['to'],  'value' => $option['value']['to']), $input_value['to']);
