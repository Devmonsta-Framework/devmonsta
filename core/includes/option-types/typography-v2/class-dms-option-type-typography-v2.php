 ...
 * Typography
 */
class DMS_Option_Type_Typography_v2 extends DMS_Option_Type {
	...
	protected function _enqueue_static( $id, $option, $data ) {
		...
		wp_enqueue_script(
			...
			array( 'jquery', 'underscore', 'dms', 'dms-selectize' ),
			dms()->manifest->get_version()
		);

		wp_localize_script(
			'dms-option-' . $this->get_type(),
			'dms_typography_v2_fonts',
			$this->get_fonts()
		);
	}
	...
	public function get_fonts() {
		$cache_key = 'dms_option_type/'. $this->get_type();

		try {
			return DMS_Cache::get($cache_key);
		} catch (DMS_Cache_Not_Found_Exception $e) {
			$fonts = array(
				'standard' => apply_filters( 'dms_option_type_typography_v2_standard_fonts', array(
					"Arial",
					"Verdana",
					"Trebuchet",
					"Georgia",
				...
				'google' => apply_filters(
					'dms_option_type_typography_v2_google_fonts',
					json_decode( dms_get_google_fonts_v2(), true )
				)
			);
