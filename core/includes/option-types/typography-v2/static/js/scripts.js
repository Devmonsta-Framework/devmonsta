/*global dms_typography_v2_fonts, jQuery, _ */
( function ($) {
	$(document).ready(function () {
		var optionTypeClass = '.dms-option-type-typography-v2',
			googleFonts = dms_typography_v2_fonts['google'],
			/**
			...
			getFontsOptions = function(){
				if (fontsOptions === null) {
					...
					_.each(dms_typography_v2_fonts['standard'], function (item) {
		...
		dmsEvents.on('dms:options:init', function (data) {
			data.$elements.find(optionTypeClass +':not(.initialized)').each(function(){
				...
				$option.find([
					'.dms-option-typography-v2-option .dms-option-typography-v2-option-size-input',
					'.dms-option-typography-v2-option .dms-option-typography-v2-option-line-height-input',
					'.dms-option-typography-v2-option .dms-option-typography-v2-option-letter-spacing-input'
				...
				{
					var $fontFamilySelect = $option.find('.dms-option-typography-v2-option-family select[data-type="family"]');
						...
						.selectize({
							onChange: function (selected) {
								var results = $.grep(googleFonts['items'], function (font) { return font['family'] === selected; }),
									$variations = $option.find('.dms-option-typography-v2-option-variation'),
									$subsets = $option.find('.dms-option-typography-v2-option-subset'),
									$style = $option.find('.dms-option-typography-v2-option-style'),
									$weight = $option.find('.dms-option-typography-v2-option-weight');
								...
								this.$wrapper.find('input[type="text"]').attr('data-dms-pressed-backspace', 'false');
							},
							onInitialize: function () {
								...
								this.$wrapper.find('input[type="text"]').on('keydown', function (e) {
									if (e.keyCode === 40) {
										$(this).attr('data-dms-pressed-backspace', 'true');
									} else {
										if ($(this).attr('data-dms-pressed-backspace') == 'false') {
