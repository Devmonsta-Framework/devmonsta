(function ($, _, dmse) {
	...
	var init = function () {
		...
		var createFrame = function () {
			...
			frame.on('ready', function () {
				frame.modal.$el.addClass('dms-option-type-multi-upload');
			});
			...
			frame.on('select', function () {
				...
				dmse.trigger('dms:option-type:multi-upload:change', {
					$element: elements.$container,
					attachments: attachments
				});

				elements.$container.trigger('dms:option-type:multi-upload:change', {
					attachments: attachments
				});
		...
		elements.$thumbsContainer.on('click', '.clear-uploads-thumb', function (e) {
			...
			if (ids.length) {
				elements.$input.val(JSON.stringify(ids));
				dmse.trigger('dms:option-type:multi-upload:remove', {$element: elements.$container}); // TODO: think what other data would be usefull
				elements.$container.trigger('dms:option-type:multi-upload:remove'); // TODO: think what other data would be usefull
			} else {
				elements.$input.val('[]');
				...
				dmse.trigger('dms:option-type:multi-upload:clear', {$element: elements.$container});
				elements.$container.trigger('dms:option-type:multi-upload:clear');
			}
	...
	dmse.on('dms:options:init', function (data) {
		data.$elements
			.find('.dms-option-type-multi-upload.images-only:not(.dms-option-initialized)').each(init)
			.addClass('dms-option-initialized');
	});

	dms.options.register('multi-upload', {
