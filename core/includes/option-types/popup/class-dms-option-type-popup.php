...
class DMS_Option_Type_Popup extends DMS_Option_Type {
	...
	protected function _enqueue_static( $id, $option, $data ) {
		...
		if ($enqueue) {
			...
			wp_enqueue_script(
				'dms-option-' . $this->get_type(),
				dms_get_framework_directory_uri( '/includes/option-types/' . $this->get_type() . '/static/js/' . $this->get_type() . '.js' ),
				array( 'underscore', 'dms-events', 'jquery-ui-sortable', 'dms' ),
				false,
				true
			);
		...
		dms()->backend->enqueue_options_static( $option['popup-options'] );
	...
	protected function _render( $id, $option, $data ) {
		...
		$sortable_image = dms_get_framework_directory_uri( '/static/img/sort-vertically.png' );

		return dms_render_view( dms_get_framework_directory( '/includes/option-types/' . $this->get_type() . '/views/view.php' ),
	...
	private function transform_options( $options ) {
		...
		foreach ( $options as $id => $option ) {
			if ( is_int( $id ) ) {
				/**
				 * this happens when in options array are loaded external options using dms()->theme->get_options()
	...
	protected function _get_value_from_input( $option, $input_value ) {
		 ...
		 * because popup <input> contains options db values got from dms.OptionsModal
		 * which can't be used as $input_value in second parameter of dms_get_options_values_from_input()
		 */
		$popup_options = array();
		foreach (dms_extract_only_options($option['popup-options']) as $popup_option_id => $popup_option) {
		...
		return dms_get_options_values_from_input($popup_options, array());
	}
