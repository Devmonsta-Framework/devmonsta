 ...
 * Map
 */
class DMS_Option_Type_Map extends DMS_Option_Type {
	...
	protected function _enqueue_static( $id, $option, $data ) {
		...
		wp_enqueue_script(
			...
			array( 'jquery', 'jquery-ui-widget', 'dms-events', 'underscore', 'jquery-ui-autocomplete' ),
			'1.0',
			true
		);
	...
	protected function _render( $id, $option, $data ) {
		...
		$path = dms_get_framework_directory(
			'/includes/option-types/' . $this->get_type() . '/views/view.php'
		);

		return dms_render_view($path, array(
			'id'     => $id,
			'option' => $option,
			'data'   => $data
		));
	...
	public static function api_key() {
		return DMS_Option_Type_GMap_Key::get_key();
	}
}
