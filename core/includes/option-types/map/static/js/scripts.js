...
(function($, _, dmse, localized) {
	jQuery(document).ready(function() {
		...
		function dms_option_map_initialize(data) {
			if (typeof google === 'undefined') {
				data.find('.dms-option-map-inputs').attr('readonly', 'readonly');
				return;
			}
			...
			var option = {
				...
				toggles: {
					expand: data.find('.dms-option-maps-expand'),
					reset: data.find('.dms-option-maps-close'),
				},

				tabs: {
					first: data.find('.dms-option-maps-tab.first'),
					second: data.find('.dms-option-maps-tab.second'),
				},
		...
		dmse.on('dms:options:init', function(data) {
			var obj = data.$elements.find(
				'.dms-option-type-map:not(.initialized)'
			);
					...
					$.ajax({
						...
						.done(function() {
							$.each(pendingInit, function(i, obj) {
								obj.each(function() {
									dms_option_map_initialize($(this));
								});
							});
...
})(jQuery, _, dmsEvents, _dms_option_type_map);
