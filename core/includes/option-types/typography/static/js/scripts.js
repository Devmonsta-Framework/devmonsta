/*global dms_typography_fonts */
( function ($) {
	$(document).ready(function () {
		var optionTypeClass = '.dms-option-type-typography',
			/**
			...
			getFontsOptions = function(){
				if (fontsOptions === null) {
					...
					_.each(dms_typography_fonts['standard'], function (item) {
					...
					_.each(dms_typography_fonts['google'], function (item) {
		...
		dmsEvents.on('dms:options:init', function (data) {
			data.$elements.find(optionTypeClass +':not(.initialized)').each(function(){
				...
				{
					var $fontFamilySelect = $option.find('.dms-option-typography-option-family select[data-type="family"]');
						...
						.selectize({
							render: {
								option: function (item) {
									if (dms_typography_fonts['google'].hasOwnProperty(item.value)) {
										var background = (typeof dms_typography_fonts['google'][item.value].position === "number")
											? 'style="background-position: 0 -' + dms_typography_fonts['google'][item.value].position + 'px;'
							...
							onChange: function (selected) {
								...
								if (dms_typography_fonts['google'].hasOwnProperty(selected)) {
									var font = dms_typography_fonts['google'][selected];
									_.each(font.variants, function (variant) {
										html += '<option value="' + variant + '">' + dms.capitalizeFirstLetter(variant) + '</option>';
									});
								} else {
								...
								this.$dropdown
									.closest('.dms-option-typography-option-family')
									.next('.dms-option-typography-option-style')
