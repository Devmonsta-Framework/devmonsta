...
class DMS_Option_Type_Date_Picker extends DMS_Option_Type {
	...
	{
		$uri = dms_get_framework_directory_uri('/includes/option-types/' . $this->get_type() . '/static');
		...
		wp_enqueue_script(
			'dms-option-' . $this->get_type() . '-date-picker',
			$date_picker_js_uri,
			array('jquery', 'dms-events'),
			dms()->manifest->get_version(),
			true
		...
		if ($locale_uri) {
			wp_enqueue_script(
				'dms-option-' . $this->get_type() . '-date-picker-locale',
				$locale_uri,
				array('dms-option-' . $this->get_type() . '-date-picker'),
				dms()->manifest->get_version(),
				true
			);
		}
	...
	protected function _render( $id, $option, $data ) {
		...
		$option['attr']['data-dms-option-date-picker-opts'] = json_encode( $properties );
