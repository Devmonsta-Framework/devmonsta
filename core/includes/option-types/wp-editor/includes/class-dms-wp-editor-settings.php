...
class DMS_WP_Editor_Manager {
	...
	public function __construct($id, $option, $data) {
		 ...
		 */
		$this->editor_id = dms()->backend->option_type('wp-editor')->get_id_prefix() . dms_rand_md5();
	}
	...
	public function get_html() {
...

		{
			...
			$option['attr']['data-dms-editor-id'] = $this->editor_id;
			$option['attr']['data-dms-mce-settings'] = json_encode($preinit_data['mce_settings']);
			$option['attr']['data-dms-qt-settings'] = json_encode($preinit_data['qt_settings']);

			if ($option['shortcodes']) {
				$option['attr']['data-dms-shortcodes-list'] = json_encode(
					$option['shortcodes']
				);
			}
