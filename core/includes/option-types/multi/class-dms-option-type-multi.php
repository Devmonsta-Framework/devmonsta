 ...
 * Rows with options
 */
class DMS_Option_Type_Multi extends DMS_Option_Type
{
	...
	protected function _enqueue_static($id, $option, $data)
	{
		...
		dms()->backend->enqueue_options_static($option['inner-options']);
	...
	protected function _render($id, $option, $data)
	{
		...
		return '<div '. dms_attr_to_html($div_attr) .'>'.
			dms()->backend->render_options($option['inner-options'], $data['value'], array(
	...
	protected function _get_value_from_input($option, $input_value)
	{
		...
		foreach (dms_extract_only_options($option['inner-options']) as $inner_id => $inner_option) {
			$value[$inner_id] = dms()->backend->option_type($inner_option['type'])->get_value_from_input(
