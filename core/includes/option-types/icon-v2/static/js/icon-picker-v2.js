;(function($) {
	window.dmsOptionTypeIconV2Picker = dms.Modal.extend({
		defaults: _.extend({}, dms.Modal.prototype.defaults, {
			title: 'Icon V2',
			size: 'small',
			modalCustomClass: 'dms-icon-v2-picker-modal',
		...
		ContentView: dms.Modal.prototype.ContentView.extend({
			events: {
				'input .dms-icon-v2-icons-library .dms-icon-v2-toolbar input':
					'onSearch',
				'click .dms-icon-v2-library-icon': 'markIconAsSelected',
				'click .dms-icon-v2-library-icon a': 'markIconAsFavorite',
				'click button.dms-icon-v2-custom-upload-perform':
					'performImageUpload',
				submit: 'onSubmit',
			},

			initialize: function() {
				dms.Modal.prototype.ContentView.prototype.initialize.call(this)
			...
			performImageUpload: function() {
				...
				uploadFrame.on('ready', function() {
					uploadFrame.modal.$el.addClass('dms-option-type-upload')
				})
			...
			markIconAsSelected: function markIconAsSelected(e) {
				...
				var type =
					$el.closest(
						'[data-dms-option-id="upload-custom-icon-recents"]'
					).length > 0
						? 'custom-upload'
						: 'icon-font'

				var result = $el.attr('data-dms-icon-v2').trim()
			...
			refreshSelectedIcon: function refreshSelectedIcon() {
				this.model.frame.$el
					.find('.dms-icon-v2-library-icon.selected')
				...
				if (currentValue) {
					this.model.frame.$el
						.find('[data-dms-icon-v2$="' + currentValue + '"]')
						.addClass('selected')
				}
			},
			...
			markIconAsFavorite: function markIconAsFavorite(e) {
				...
				var icon = $(e.currentTarget)
					.closest('.dms-icon-v2-library-icon')
					.attr('data-dms-icon-v2')
			...
			refreshFavorites: function() {
				$('.dms-icon-v2-favorite').removeClass('dms-icon-v2-favorite')
				...
				_.map(this.model.currentFavorites, function(favorite) {
					...
					$('[data-dms-icon-v2="' + favorite + '"]').addClass(
						'dms-icon-v2-favorite'
					)
				})
			},
			...
			renderFavoritesAndRecentUploads: function() {
				this.model.frame.$el
					.find('.dms-favorite-icons-wrapper')
					.replaceWith(this.model.getFavoritesHtml())

				this.model.frame.$el
					.find(
						'[data-dms-option-id="upload-custom-icon-recents"] .dms-option-html'
					)
		...
		initialize: function(attributes, settings) {
			dms.Modal.prototype.initialize.call(this, attributes, {
				disableResetButton: true,
			})
		...
		initializeFrame: function(settings) {
			dms.OptionsModal.prototype.initializeFrame.call(this, settings)
		},
		...
		close: function() {
			dms.Modal.prototype.close.call(this)
		},

		prepareForPick: function() {
			var modal = this

			modal.frame.$el.find('.dms-icon-v2-toolbar select').selectize({
				plugins: ['hidden_textfield'],
				onChange: _.bind(modal.applyFilters, modal),
			})

			modal.frame.$el
				.find('.dms-options-tabs-wrapper')
				.off('tabsactivate.dmsiconv2')
				.on('tabsactivate.dmsiconv2', function(event, ui) {
					/**
			...
			if (modal.get('current_state').type !== 'custom-upload') {
				...
				if (modal.result['icon-class']) {
					this.frame.$el
						.find(
							'.dms-icon-v2-icons-library .dms-icon-v2-toolbar input.dms-option-type-text'
						)
						.val('')
					...
					var selectInput = modal.frame.$el.find(
						'.dms-icon-v2-icons-library .dms-icon-v2-toolbar select'
					)[0]
		...
		applyFilters: function() {
			...
			this.frame.$el
				.find(
					'[data-dms-option-id="icon-font"] .dms-icon-v2-library-pack-wrapper'
				)
				.html(
					wp.template('dms-icon-v2-packs')({
						packs: packs,
		...
		loadLatestFavorites: function() {
			...
			var ajaxPromise = $.post(ajaxurl, {
				action: 'dms_icon_v2_get_favorites',
			})
		...
		syncFavoritesToServer: function() {
			jQuery.post(ajaxurl, {
				action: 'dms_icon_v2_update_favorites',
		...
		getTabsHtml: function() {
			return wp.template('dms-icon-v2-tabs')({
		...
		getLibraryHtml: function() {
			...
			return wp.template('dms-icon-v2-library')({
				packs: _.values(this.getIconsData()),
		...
		getFavoritesHtml: function() {
			return wp.template('dms-icon-v2-favorites')({
				favorites: this.currentFavorites || [],
				current_state: this.result,
			})
		},
		...
		getRecentIconsHtml: function() {
			return wp.template('dms-icon-v2-recent-custom-icon-uploads')({
				favorites: this.currentFavorites || [],
				current_state: this.result,
			})
		},
	...
	$(function() {
		dmsOptionTypeIconV2Instance = new dmsOptionTypeIconV2Picker()
	})
