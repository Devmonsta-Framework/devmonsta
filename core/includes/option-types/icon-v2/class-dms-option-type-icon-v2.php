...
class DMS_Option_Type_Icon_v2 extends DMS_Option_Type {
	...
	public function _init() {
		 ...
		 *
		 * dms()->backend->option_type('icon-v2')->packs_loader->enqueue_frontend_css()
		 */
		$this->packs_loader = new DMS_Icon_V2_Packs_Loader();

		if (! is_admin()) { return; }

		$this->favorites = new DMS_Icon_V2_Favorites_Manager();
	}
	...
	protected function _enqueue_static($id, $option, $data) {
		...
		$static_URI = dms_get_framework_directory_uri(
			'/includes/option-types/' . $this->get_type() . '/static/'
		);

		wp_enqueue_style('dms-selectize');

		wp_enqueue_script(
			'dms-option-type-'. $this->get_type() .'-backend-previews',
			$static_URI . 'js/render-icon-previews.js',
			array('jquery', 'dms', 'dms-events', 'dms-selectize'),
			dms()->manifest->get_version()
		);

		wp_enqueue_script(
			'dms-option-type-'. $this->get_type() .'-backend-picker-v2',
		...
		wp_enqueue_style(
			'dms-option-type-'. $this->get_type() .'-backend-picker',
		...
		wp_localize_script(
			...
			'dms_icon_v2_data',
			array(
				'edit_icon_label' => __('Change Icon', 'dms'),
				'add_icon_label' => __('Add Icon', 'dms')
			)
		);
	}

	public function load_templates() {
		echo dms_render_view(
