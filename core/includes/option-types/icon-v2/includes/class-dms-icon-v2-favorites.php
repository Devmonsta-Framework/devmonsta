...
class DMS_Icon_V2_Favorites_Manager
{
	private $key = 'dms-icon-v2-favorites';

	public function attach_ajax_actions()
	{
		add_action(
			'wp_ajax_dms_icon_v2_update_favorites',
			array($this, 'set_favorites_action')
		);

		add_action(
			'wp_ajax_dms_icon_v2_get_favorites',
			array($this, 'get_favorites_action')
		);

		add_action(
			'wp_ajax_dms_icon_v2_get_icons',
	...
	public function get_icon_packs() {
		wp_send_json_success(
			dms()->backend->option_type('icon-v2')->packs_loader->get_packs(true)
		);
	}

	public function set_favorites_action()
	{
		$favorites = json_decode(DMS_Request::POST( 'favorites' ), true);
