...
class DMS_Icon_V2_Packs_Loader
{
	...
	public function enqueue_frontend_css()
	{
		foreach ($this->get_packs() as $pack_name => $pack) {
			 ...
			 * `dms:option-type:icon-v2:packs` filter.
			 *
			...
			if ($pack['require_css_file']) {
				...
				wp_enqueue_style(
					'dms-option-type-icon-v2-pack-' . $pack_name,
					$pack['css_file_uri'],
					array(),
	...
	public function _default_load_icons_callback($pack)
	{
		...
		if (is_array($pack['icons'])) {
			return array();
		}

		$css = $wp_filesystem->get_contents($pack['css_file']);
	...
	{
		if ($this->icons_for_packs_parsed) {
			return;
		}
	...
	private function _load_filtered_icon_packs()
	{
		...
		$packs = apply_filters(
			'dms:option_type:icon-v2:filter_packs',
	...
	public function get_default_icon_packs()
	{
		return array(
			...
			'linecons' => array(
				...
				'css_file' => dms_get_framework_directory(
					'/static/libs/linecons/css/linecons.css'
				),

				'css_file_uri' => dms_get_framework_directory_uri(
					'/static/libs/linecons/css/linecons.css'
				),
			),
			...
			'unycon' => array(
				...
				'css_file' => dms_get_framework_directory('/static/libs/unycon/unycon.css'),
				'css_file_uri' => dms_get_framework_directory_uri(
					'/static/libs/unycon/unycon.css'
				),

				'admin_wp_enqueue_handle' => 'dms-unycon'
			)
		);
	}
