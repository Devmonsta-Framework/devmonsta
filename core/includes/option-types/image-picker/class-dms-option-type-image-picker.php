...
class Dms_Option_Type_Image_Picker extends DMS_Option_Type
{
	...
	{
		wp_enqueue_script(
			'dms-option-' . $this->get_type() . '-image-picker',
			dms_get_framework_directory_uri('/includes/option-types/' . $this->get_type() . '/static/js/image-picker/image-picker.js'),
			array(),
		...
		$html = ''; {
			...
			foreach ($option['choices'] as $key => $choice) {
				...
				if (!empty($choice['large'])) {
					...
					$pre_load_images_html .= dms_html_tag('img', $choice['large']);
				}
				...
				$html .= dms_html_tag('option', $attr, dms_htmlspecialchars(isset($choice['label']) ? $choice['label'] : ''));
			}
