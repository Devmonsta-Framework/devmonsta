 ...
 * -*==*---
 */
class DMS_Option_Type_Range_Slider extends DMS_Option_Type {
	...
	protected function _enqueue_static( $id, $option, $data ) {
		{
			wp_enqueue_style(
				'dms-option-' . $this->get_type() . 'ion-range-slider',
				dms_get_framework_directory_uri( '/includes/option-types/' . $this->get_type() . '/static/libs/ion-range-slider/ion.rangeSlider.css' ),
				dms()->manifest->get_version()
			);

			wp_enqueue_script(
				'dms-option-' . $this->get_type() . 'ion-range-slider',
				dms_get_framework_directory_uri( '/includes/option-types/' . $this->get_type() . '/static/libs/ion-range-slider/ion.rangeSlider.min.js' ),
		...
		wp_enqueue_script(
			...
			array( 'jquery', 'dms-events', 'underscore', 'dms-option-' . $this->get_type() . 'ion-range-slider' ),
			dms()->manifest->get_version()
		);
	}
	...
	protected function _render( $id, $option, $data ) {
		...
		return dms_render_view( dms_get_framework_directory( '/includes/option-types/' . $this->get_type() . '/view.php' ),
			array(
				'id'     => $id,
				'option' => $option,
				'data'   => $data,
