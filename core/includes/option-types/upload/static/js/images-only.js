(function($, _, dmse) {
	...
	var init = function() {
		var $this = $(this),
			elements = {
				...
				$urlInput: $this.find('input[type="hidden"].dms-option-type-upload-image-url'),
		...
		var createFrame = function() {
			...
			frame.on('ready', function() {
				frame.modal.$el.addClass('dms-option-type-upload');
			});
		...
		function clearAttachment () {
			...
			dmse.trigger('dms:option-type:upload:clear', {$element: elements.$container});
			elements.$container.trigger('dms:option-type:upload:clear');

			dms.options.trigger.changeForEl(elements.$container, {
				value: {}
			});

			dms.options.trigger.scopedByType('clear', elements.$container, {
				value: {}
			});
		}
		...
		function performSelection (attachment) {
			...
			dmse.trigger('dms:option-type:upload:change', {
				$element: elements.$container,
				attachment: attachment
			});

			elements.$container.trigger('dms:option-type:upload:change', {
				attachment: attachment
			});
	...
	dmse.on('dms:options:init', function(data) {
		data.$elements
			.find('.dms-option-type-upload.images-only:not(.dms-option-initialized)').each(init)
			.addClass('dms-option-initialized');
	});

	dms.options.register('upload', {
