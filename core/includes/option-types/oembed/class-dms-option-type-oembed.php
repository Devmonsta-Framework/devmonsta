...
class DMS_Option_Type_Oembed extends DMS_Option_Type {
	...
	protected function _enqueue_static( $id, $option, $data ) {
		...
		wp_enqueue_script(
			...
			array( 'underscore', 'dms-events', 'dms', 'wp-util' ),
			false,
			true
		);
	}
	...
	public static function _action_get_oembed_response() {
		...
		if ( wp_verify_nonce( DMS_Request::POST( '_nonce' ), '_action_get_oembed_response' ) ) {

			$url        = DMS_Request::POST( 'url' );
			$width      = DMS_Request::POST( 'preview/width' );
			$height     = DMS_Request::POST( 'preview/height' );
			$keep_ratio = ( DMS_Request::POST( 'preview/keep_ratio' ) === 'true' );

			$iframe = empty( $keep_ratio ) ?
				dms_oembed_get( $url, compact( 'width', 'height' ) ) :
...
add_action(
	...
	array( "DMS_Option_Type_Oembed", '_action_get_oembed_response' )
);