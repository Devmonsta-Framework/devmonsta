...
class DMS_Option_Type_Datetime_Picker extends DMS_Option_Type {
	...
	protected function _render( $id, $option, $data ) {
		...
		$wrapper_attr['data-min-date'] = dms_akg('datetime-picker/minDate', $option, false );
		$wrapper_attr['data-max-date'] = dms_akg('datetime-picker/maxDate', $option, false );
		...
		$html = '<div ' . dms_attr_to_html($wrapper_attr) .' >';
		$html .= dms()->backend->option_type( 'text' )->render( $id, $option, $data );
		$html .= '</div>';
	...
	protected function _enqueue_static($id, $option, $data) {
		//plugin styles & js
		{
			$css_lib_uri        = dms_get_framework_directory_uri('/includes/option-types/datetime-picker/static/css/jquery.datetimepicker.css');
			$js_lib_uri         = dms_get_framework_directory_uri('/includes/option-types/datetime-picker/static/js/jquery.datetimepicker.js');
		}

		//framework styles & js
		{
			$css_main_uri    = dms_get_framework_directory_uri('/includes/option-types/datetime-picker/static/css/style.css');
			$js_main_uri     = dms_get_framework_directory_uri('/includes/option-types/datetime-picker/static/js/script.js');
		}

		wp_enqueue_style( 'dms-option-datetime-picker-lib-css', $css_lib_uri );
		wp_enqueue_style( 'dms-option-datetime-picker-main-css', $css_main_uri );
		wp_enqueue_script( 'dms-moment' );
		wp_enqueue_script( 'dms-option-datetime-picker-lib-js', $js_lib_uri, array('jquery', 'dms-moment'), false, true );
		wp_enqueue_script( 'dms-option-datetime-picker-main-js', $js_main_uri, array('jquery', 'dms-option-datetime-picker-lib-js', 'dms-events' ), false, true );

		dms()->backend->option_type( 'text' )->enqueue_static();
	}
	...
	protected function _get_value_from_input($option, $input_value)
	{
		...
		if ( isset($option['datetime-picker']['format']) && !empty($option['datetime-picker']['format']) ){
			if ($this->_dms_validate_date_format($input_value, $option['datetime-picker']['format'], $extra_formats ) === false) {
				return $option['value'];
			}
		}
	...
	private function _dms_validate_date_format($date, $format = 'Y/m/d H:i', $extra_formats = array())
	{
