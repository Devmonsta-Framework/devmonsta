 ...
 */
class DMS_Option_Type_Hidden extends DMS_Option_Type
{
	...
	protected function _render($id, $option, $data)
	{
		...
		return '<input ' . dms_attr_to_html($option['attr']) . ' type="hidden" />';
	}
...
class DMS_Option_Type_Text extends DMS_Option_Type
{
	public function get_type()
	{
		return 'text';
	...
	protected function _render($id, $option, $data)
	{
		...
		return '<input ' . dms_attr_to_html($option['attr']) . ' type="text" />';
	}
...
class DMS_Option_Type_Short_Text extends DMS_Option_Type_Text
{
	public function get_type()
	{
		return 'short-text';
	...
	protected function _render($id, $option, $data)
	{
		$option['attr']['class'] .= ' dms-option-width-short';
...
class DMS_Option_Type_Password extends DMS_Option_Type
{
	public function get_type()
	{
		return 'password';
	...
	protected function _render($id, $option, $data)
	{
		...
		return '<input ' . dms_attr_to_html($option['attr']) . ' type="password" />';
	}
...
class DMS_Option_Type_Textarea extends DMS_Option_Type
{
	public function get_type()
	{
		return 'textarea';
	...
	protected function _render($id, $option, $data)
	{
		...
		return '<textarea ' . dms_attr_to_html($option['attr']) . '>' .
...
class DMS_Option_Type_Html extends DMS_Option_Type
{
	public function get_type()
	{
		return 'html';
	...
	protected function _render($id, $option, $data)
	{
		...
		return '<div ' . dms_attr_to_html($div_attr) . '>' .
			dms()->backend->option_type('hidden')->render(
				$id,
				array(
					'attr'  => array(
						'class' => 'dms-option-html-value',
					),
			...
			'<div class="dms-option-html">' . $option['html'] . '</div>' .
			'</div>';
	}
 ...
 */
class DMS_Option_Type_Html_Fixed extends DMS_Option_Type_Html
{
	public function get_type()
	{
		return 'html-fixed';
 ...
 */
class DMS_Option_Type_Html_Full extends DMS_Option_Type_Html
{
	public function get_type()
	{
		return 'html-full';
...
class DMS_Option_Type_Checkbox extends DMS_Option_Type
{
	public function get_type()
	{
		return 'checkbox';
	...
	protected function _render($id, $option, $data)
	{
		if (
			...
			/**
			 * This happens on devmonsta.OptionsModal open/render
		...
		return '' .
			...
			'<input ' . dms_attr_to_html($option['attr']) . ' type="checkbox" value="true" ' .
 ...
 * Checkboxes list
 */
class DMS_Option_Type_Checkboxes extends DMS_Option_Type
{
	public function get_type()
	{
		return 'checkboxes';
	...
	protected function _render($id, $option, $data)
	{
		...
		if ($option['inline']) {
			$div_attr['class'] .= ' devmonsta-option-type-checkboxes-inline devmonsta-clearfix';
		}

		$html = '<div ' . dms_attr_to_html($div_attr) . '>';
		...
		foreach ($option['choices'] as $value => $choice) {
			...
			$choice['attr'] = array_merge(
				...
				array(
					...
					'data-devmonsta-checkbox-id' => $value
				),
			...
			$html .=
				...
				'<input  ' . dms_attr_to_html($choice['attr']) . '>' .
 ...
 * Radio list
 */
class DMS_Option_Type_Radio extends DMS_Option_Type
{
	public function get_type()
	{
		return 'radio';
	...
	protected function _render($id, $option, $data)
	{
		...
		if ($option['inline']) {
			$div_attr['class'] .= ' devmonsta-option-type-radio-inline devmonsta-clearfix';
		}
 ...
 * Select
 */
class DMS_Option_Type_Select extends DMS_Option_Type
{
	public function get_type()
	{
		return 'select';
	...
	protected function _render($id, $option, $data)
	{
		...
		$html = '<select ' . dms_attr_to_html($option['attr']) . '>' .
		...
		foreach ($choices as $c_value => $choice) {
			if (is_array($choice)) {
				...
				if (isset($choice['choices'])) { // optgroup
					$html .=
						'<optgroup ' . dms_attr_to_html($choice['attr']) . '>' .
					...
					$html .=
						'<option ' . dms_attr_to_html($choice['attr']) . selected($c_value, $value, false) . '>' .
...
class DMS_Option_Type_Short_Select extends DMS_Option_Type_Select
{
	public function get_type()
	{
		return 'short-select';
	...
	protected function _render($id, $option, $data)
	{
		$option['attr']['class'] .= ' devmonsta-option-width-short';
 ...
 * Select Multiple
 */
class DMS_Option_Type_Select_Multiple extends DMS_Option_Type_Select
{
	public function get_type()
	{
	...
	protected function render_choices($choices, $value)
	{
		...
		foreach ($choices as $c_value => $choice) {
			if (is_array($choice)) {
				...
				if (isset($choice['choices'])) { // optgroup
					$html .= '<optgroup ' . dms_attr_to_html($choice['attr']) . '>' .
						$this->render_choices($choice['choices'], $value) .
						'</optgroup>';
				} else { // choice as array (with custom attributes)
					$choice['attr']['value'] = $c_value;
					...
					$html .= '<option ' . dms_attr_to_html($choice['attr']) . ' ' .
...
class DMS_Option_Type_Unique extends DMS_Option_Type
{
	public static $cache_key = 'devmonsta:option-type:unique_id:ids';
	...
	protected function _render($id, $option, $data)
	{
		return devmonsta_html_tag('input', array(
			'type' => 'hidden',
			'name' => $option['attr']['name'],
			'id' => $option['attr']['id'],
			'value' => $data['value'],
	...
	protected function _init()
	{
		...
		add_filter('devmonsta:option-type:addable-popup:value-from-input', array($this, '_filter_addable_popup_value_from_input'), 10, 2);
	}
	...
	public function _action_reset_post_ids($post_id)
	{
		...
		if (isset($ids[$post_id])) {
			DMS_Cache::set(self::$cache_key . '/' . $post_id, $ids);
		}

		if (isset($ids[$original_id])) {
			DMS_Cache::set(self::$cache_key . '/' . $original_id, $ids);
		}
	}
	...
	protected function _get_value_from_input($option, $input_value)
	{
		...
		if (self::$should_do_regeneration) {
			...
			DMS_Cache::set(self::$cache_key, $ids);
		}
	...
	{
		try {
			return DMS_Cache::get($cache_key = 'devmonsta:option-type:unique_id:ids');
		} catch (DMS_Cache_Not_Found_Exception $e) {

			DMS_Cache::set($cache_key, array());
	...
	public function _filter_addable_popup_value_from_input($value, $option)
	{
		 ...
		 */ {
			...
			devmonsta_collect_options($update_options, $option['popup-options'], array(
		...
		foreach ($value as &$row) {
			foreach ($update_options as $opt_id => $opt) {
				if (isset($row[$opt_id])) { // should not happen, but just in case, prevent notice
					$row[$opt_id] = dms()->backend->option_type($opt['type'])->get_value_from_input(
 ...
 * @since 2.5.7
 */
class DMS_Option_Type_GMap_Key extends DMS_Option_Type_Text
{
	...
	{
		return 'devmonsta-option-types:gmap-key';
	}
