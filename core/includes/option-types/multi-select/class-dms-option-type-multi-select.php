...
if ( ! class_exists( 'DMS_Option_Type_Multi_Select' ) ):

	/**
	 * Select multiple choices from different sources: posts, taxonomies, users or custom array
	 */
	class DMS_Option_Type_Multi_Select extends DMS_Option_Type {
		/**
		 * @internal
		 */
		...
		private static function query_posts( array $options ) {
			...
			return self::get_posts( $ids, max( dms_akg( 'limit', $options, 100 ), 1 ) );
		}
		...
		protected static function build_post( $id, $show_type ) {
			$title = ( $t = get_the_title( $id ) ) && $t ? $t : esc_html__( 'No title', 'dms' ) . ' - #' . $id;
		...
		private static function query_terms( array $options ) {
			...
			return self::get_terms( $ids, array_keys( $limits['taxonomy'] ), max( dms_akg( 'limit', $options, 100 ), 1 ) );
		}
		...
		public static function _ajax_autocomplete() {
			...
			$type  = DMS_Request::POST( 'data/type' );
			$names = ( $names = json_decode( DMS_Request::POST( 'data/names' ), true ) ) ? $names : array();
			$title = DMS_Request::POST( 'data/string' );
			$show  = DMS_Request::POST( 'data/show-type', false );
		...
		protected function _render( $id, $option, $data ) {
			...
			$source     = array();
			$items      = array();
			$show       = dms_akg( 'show-type', $option, false );
			...
			if ( isset( $option['population'] ) ) {
				...
				$option['attr']['data-show-type']  = (int) dms_akg( 'show-type', $option, false );
			...
			return dms()->backend->option_type( 'text' )->render( $id, $option, $data );
		}
		...
		protected function _enqueue_static( $id, $option, $data ) {
			wp_enqueue_style(
				...
				dms_get_framework_directory_uri( '/includes/option-types/' . $this->get_type() . '/static/css/style.css' ),
				array( 'dms-selectize' ),
			...
			wp_enqueue_script(
				...
				array( 'jquery', 'dms-events', 'dms-selectize' ),
				dms()->manifest->get_version(),
				true
			);
		...
		private static function get_post_type_name( $type ) {
			...
			if ( ! isset( $names[ $type ] ) ) {
				$names[ $type ] = dms_akg( 'labels/name', get_post_type_object( $type ), _x( 'Unknown', 'unknown-post-type', 'dms' ) );
			}
		...
		private static function get_tax_name( $tax ) {
			...
			if ( ! isset( $names[ $tax ] ) ) {
				$names[ $tax ] = dms_akg( 'labels/name', get_taxonomy( $tax ), _x( 'Unknown', 'unknown-post-type', 'dms' ) );
			}
		...
		private static function filter_query_posts( $set ) {
			...
			return apply_filters( 'dms:option-type:multi-select:query_posts', $set );
		}
	}

	add_action( 'wp_ajax_dms_option_type_multi_select_autocomplete',
		array( "DMS_Option_Type_Multi_Select", '_ajax_autocomplete' ) );
endif;