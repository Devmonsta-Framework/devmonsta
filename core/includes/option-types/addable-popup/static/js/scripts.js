(function ($, _, dmsEvents, window) {
				...
				initSortable: function () {
					...
					nodes.$itemsWrapper.sortable({
						...
						update: function(){
							...
							dms.options.trigger.changeForEl(nodes.$optionWrapper);
						},
						start: function(e, ui){
		...
		_.map(
			...
			function (ev) {
				...
				function triggerEvent (eventName, modal) {
					eventName = 'dms:option-type:addable-popup:options-modal:' + eventName;
					dmsEvents.trigger(eventName, { modal: this });
				}
			}
		);

		$this.on('remove', function(){ 
			utils.modal.frame.$el.closest('.dms-modal').remove(); // remove modal from DOM
	...
	dmsEvents.on('dms:options:init', function (data) {
		data.$elements
			.find('.dms-option-type-addable-popup:not(.dms-option-initialized)').each(addablePopup)
			.addClass('dms-option-initialized');
	});

	dms.options.register('addable-popup', {
		getValue: function (optionDescriptor) {
			var promise = $.Deferred();

			dms.whenAll(
				$(optionDescriptor.el).find(
					'> .dms-option-type-addable-popup > .items-wrapper'
				).first().find(
					'> .item.dms-backend-options-virtual-context'
