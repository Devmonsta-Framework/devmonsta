...
dms.LOADER_URI = _dms_localized.LOADER_URI;

/**
 * Useful images
 */
dms.img = {
	loadingSpinner: dms.SITE_URI +'/wp-admin/images/spinner.gif',
	logoSvg: dms.LOADER_URI
};
 ...
 */
dms.capitalizeFirstLetter = function(str) {
 ...
 * get resolved.
 */
dms.whenAll = function(deferreds) {
 ...
 * Usage:
 * var obj = {foo: {}};
 * dms.ops('foo/bar', 'ok', obj); // {foo: {bar: 'ok'}}
 *
 ...
 */
dms.ops = function(properties, value, obj, delimiter) {
	delimiter = delimiter || '/';
	...
	if (properties.length) {
		...
		if (typeof obj[property] == 'undefined') {
			...
			console.warn(
				'[dms.ops] Object property "'+ property +'" already has non object value:', obj[property],
			...
			if ((/^[0-9]+$/).test(property)) {
				...
				console.warn(
					'[dms.ops] Try to set non numeric property "'+ property +'" to array:', obj[property],
		...
		dms.ops(properties, value, obj[property], delimiter);
	} else {
		obj[property] = value;
	}
 ...
 * @param {String} [delimiter] Default '/'
 */
dms.opg = function(properties, obj, defaultValue, delimiter) {
	delimiter = delimiter || '/';
	...
	if (properties.length) {
		...
		return dms.opg(properties, obj[property], defaultValue, delimiter);
	} else {
		return obj[property];
	}
};
 ...
 * @returns {string}
 */
dms.getQueryString = function(name) {
...
(function(){
	 ...
	 *
	 * Usage:
	 * var modal = new dms.Modal();
	 *
	 * You can add a custom CSS class to your dms.Modal this way:
	 *
	 * var modal = new dms.Modal ({
	 ...
	 */
	dms.Modal = Backbone.Model.extend({
		defaults: {
			/* Modal title */
			title: 'Edit Options',
			headerElements: '',
		...
		ContentView: Backbone.View.extend({
			...
			render: function() {
				...
				if (this.model.get('html').length) {
					dmsEvents.trigger('dms:options:init', {$elements: this.$el});
			...
			renderSizeClass: function () {
				...
				function formSizeClass (size) { return 'dms-modal-' + size; }
			},
			initialize: function() {
			...
			afterHtmlReplaceFixes: function() {
				...
				{
					// hide last border
					this.$el.prepend('<div class="dms-backend-options-last-border-hider"></div>');

					// hide last border from tabs
					this.$el.find('.dms-options-tabs-contents > .dms-inner > .dms-options-tab')
						.append('<div class="dms-backend-options-last-border-hider"></div>');
				}

				this.$el.append('<input type="submit" class="dms-hidden hidden-submit" />');
				...
				if (this.model.get('disableLazyTabs')) {
					dmsEvents.trigger('dms:options:init:tabs', {
		...
		initializeFrame: function(settings) {
			...
			this.frame.once('ready', function(){
				...
				$modalWrapper.addClass('dms-modal');
				...
				if (_.indexOf(['large', 'medium', 'small'], size) !== -1) {
					$modalWrapper.addClass('dms-modal-' + size);
				} else {
					$modalWrapper.addClass('dms-modal-' + modal.defaults.size);
				}
				 ...
				 */
				(function(){
					var eventsNamespace = '.dmsModalCloseEffect';
					...
					var closeEffect = function(){
						...
						// begin css animation
						$modalWrapper.addClass('dms-modal-closing');
						...
						closingTimeout = setTimeout(
							function(){
								...
								$modalWrapper.removeClass('dms-modal-closing');
			...
			this.frame.on('open', function() {
				...
				// Adjust modal level related properties
				{
					...
					$modalWrapper
						.removeClass( // 'dms-modal-level-0 dms-modal-level-1 ...'
							Array.apply(null, {length: 10})
								.map(Number.call, Number)
								.map(function(i){ return 'dms-modal-level-'+ i; })
								.join(' ')
						)
						.addClass('dms-modal-level-'+ stackSize);
				...
				$modalWrapper.addClass('dms-modal-open');

				/**
				 * We probably don't need this class anymore due to the
				 * fact that we hacked CSS specificity away.
				 *
				 * I'll be keeping it here for integrity with dms.soleModal.
				 */
				$modalWrapper.addClass('dms-modal-opening');

				setTimeout(function () {
					$modalWrapper.removeClass('dms-modal-opening');
				}, 300);

				modalsStack.push($modalWrapper.find('.media-modal'));

				// Resize .dms-options-tabs-contents to fit entire window
				{
			...
			this.frame.on('close', function(){
				...
				modal.frame.modal.$el.removeClass('dms-modal-open');
		...
		resizeTabsContent: function () {
			...
			$content = this.frame.$el.find('.dms-options-tabs-first-level > .dms-options-tabs-contents');
			if ($content.length == 0) {
				return;
			}
 ...
 *     });
 */
dms.getValuesFromServer = function (data) {
	var opts = _.extend({
		options: [],
		actualValues: ""
	}, data);
	...
	var dataToSend = [
		...
		'name_prefix=dms_edit_options_modal'
	];
...
(function(){
	var dmsLoadingId = 'dms-options-modal';

	/**
	 * Modal to edit backend options
	 *
	 * Usage:
	 * var modal = new dms.OptionsModal({
	 *  title: 'Custom Title',
	 *  options: [
	 *      {'test1': {
	 *          'type': 'text',
	 ...
	 * modal.open();
	 */
	dms.OptionsModal = dms.Modal.extend({
		ContentView: dms.Modal.prototype.ContentView.extend({
			events: {
				'submit': 'onSubmit'
			},
			onSubmit: function(e) {
				...
				var loadingId = dmsLoadingId +':submit',
					view = this;

				dms.loading.show(loadingId);
				 ...
				 */
				dmsEvents.trigger('dms:options:init:tabs', {$elements: view.$el});

				view.model.getValuesFromServer(view.$el.serialize())
					.done(function (response, status, xhr) {
						dms.loading.hide(loadingId);
						...
						if (! view.model.frame.$el.hasClass('dms-options-modal-no-close')) {
							// simulate click on close button to fire animations
							view.model.frame.modal.$el.find('.media-modal-close').trigger('click');
						}

						view.model.frame.$el.removeClass('dms-options-modal-no-close');
					})
					.fail(function (xhr, status, error) {
						dms.loading.hide(loadingId);
			...
			resetForm: function() {
				var loadingId = dmsLoadingId +':reset';
		...
		defaults: _.extend(
			 ...
			 */
			{},
			dms.Modal.prototype.defaults,
			{
				/**
				 * Will be transformed to array with json_decode($options, true)
				 * and sent to dms()->backend->render_options()
				 */
				options: [
					{'demo-text': {
						'type': 'text',
				 ...
				 * also used in dms()->backend->render_options()
				 */
				values: {},
		...
		initialize: function () {
			dms.Modal.prototype.initialize.apply(this, arguments);

			dmsEvents.trigger('dms:options-modal:after-initialize', {modal: this});

			// Forward events to dmsEvents
			{
				/** @since 2.6.14 */
				this.on('open',  function () { dmsEvents.trigger('dms:options-modal:open',  {modal: this}); });

				/** @since 2.6.14 */
				this.on('close', function () { dmsEvents.trigger('dms:options-modal:close', {modal: this}); });
			}
		},
		initializeFrame: function(settings) {
			dms.Modal.prototype.initializeFrame.call(this, settings);
			...
			var modal = this,
				buttons = [
					{
						style: 'primary',
						text: settings.saveText || _dms_localized.l10n.modal_save_btn,
						priority: 40,
						click: function () {
							if (settings.shouldSaveWithoutClose) {
								modal.frame.$el.addClass('dms-options-modal-no-close');
							}
			...
			if (!(
				typeof settings.disableResetButton === 'undefined'
					? _dms_localized.options_modal.default_reset_bnt_disabled
					: settings.disableResetButton
			)) {
				buttons = buttons.concat([{
					style: '',
					text: _dms_localized.l10n.reset,
					priority: -1,
					click: function () {
			...
			if (settings.saveWithoutCloseButton) {
				buttons = buttons.concat([{
					style: '',
					text: _dms_localized.l10n.apply,
					priority: 45,
					click: function () {
						modal.frame.$el.addClass('dms-options-modal-no-close');
						triggerSubmit();
			...
			this.frame.once('ready', _.bind(function() {
				...
				this.frame.modal.$el.addClass('dms-options-modal');
			}, this));
		...
		open: function(values) {
			dms.Modal.prototype.open.call(this);
		...
		getValuesFromServer: function (actualValues) {
			return dms.getValuesFromServer({
		...
		updateHtml: function(values) {
			...
			var promise = dms.options.fetchHtml(
				this.get('options'),
 ...
 * jQuery('a:dms-external')
 */
jQuery.expr[':']['dms-external'] = function(obj){
 ...
 * @return {Boolean}
 */
dms.elementEventHasListenerInContainer = function ($element, event, $container) {
	"use strict";
 ...
 * dms.soleModal.show('unique-id', 'Hello World!');
 *
 * // open modal with close button but auto hide it after 3 seconds
 * dms.soleModal.show('unique-id', 'Hello World!', {autoHide: 3000});
 *
 * dms.soleModal.hide('unique-id');
 */
dms.soleModal = (function(){
	var inst = {
		queue: [
			/*
			{
		...
		animationTime: 300,
		$modal: null,
		backdropOpacity: 0.7, // must be the same as in .dms-modal style
		currentMethod: '',
		...
		lazyInit: function(){
			...
			this.$modal = jQuery(
				'<div class="dms-modal dms-sole-modal" style="display:none;">'+
				...
				'                <td valign="middle" class="dms-sole-modal-content dms-text-center"><!-- modal content --></td>'+
				'            </tr><tbody></table>'+
				'        </div>'+
				'    </div>'+
		...
		$getContent: function() {
			return this.$modal.find('.dms-sole-modal-content:first');
		},
		wrapWithTable: function ($modal) {
			var temporaryContent = $modal.find('.dms-sole-modal-content').html();

			$modal.find('.dms-sole-modal-content').remove();

			var htmlTemplate =
				'<tbody>' +
					'<tr>' +
						'<td valign="middle" class="dms-sole-modal-content dms-text-center">' +
							temporaryContent +
						'</td>' +
					'</tr>' +
				'</tbody>';
		...
		unwrapWithTable: function ($modal) {
			...
			$modal.find('.dms-sole-modal-content')
				.closest('table')
				.remove();

			$modal.find('.media-modal-content')
				.append(jQuery('<div>', {
					class: 'dms-sole-modal-content',
		...
		setSize: function(width, height) {
			...
			$modal.addClass('dms-modal-opening');
			...
			setTimeout(function () {
				$modal.removeClass('dms-modal-opening');
			}, this.animationTime);
		...
		show: function (id, html, opts) {
			...
			{
				this.$modal.removeClass('dms-modal-backdrop-light dms-modal-backdrop-dark');

				if (this.current.backdrop !== null) {
					this.$modal.addClass('dms-modal-backdrop-'+ (this.current.backdrop ? 'light' : 'dark'));
				}
			}

			this.$modal.removeClass('dms-modal-closing');
			this.$modal.addClass('dms-modal-open');
		...
		hide: function(id) {
			...
			this.$modal.addClass('dms-modal-closing');
			...
			this.currentMethodTimeoutId = setTimeout(_.bind(function(){
				...
				this.$modal.removeClass('dms-modal-open');
				this.$modal.removeClass('dms-modal-closing');
	...
	return {
		...
		renderFlashMessages: function (flashMessages) {
			...
			jQuery.each(flashMessages, function(type, messages){
				...
				switch (type) {
					case 'error':
						typeMessageClass = 'dms-text-danger';
						typeIconClass = 'dashicons dashicons-dismiss';
						typeTitle = _dms_localized.l10n.ah_sorry;
						break;
					case 'warning':
						typeMessageClass = 'dms-text-warning';
						typeIconClass = 'dashicons dashicons-no-alt';
						typeTitle = _dms_localized.l10n.ah_sorry;
						break;
					case 'success':
						typeMessageClass = 'dms-text-success';
						typeIconClass = 'dashicons dashicons-star-filled';
						typeTitle = _dms_localized.l10n.done;
						break;
					case 'info':
						typeMessageClass = 'dms-text-info';
				...
				jQuery.each(messages, function(messageId, message){
					...
					typeHtml.push(
						'<li>'+
							...
							'<p class="dms-text-muted"><em>'+ message +'</em></p>'+
						'</li>'
					);
				});
 ...
 * Usage:
 *
 * var confirm = dms.soleConfirm.create();
 ...
 *
 * TODO:
 *  1. Maybe pass unknown options to dms.soleModal itself.
 */
dms.soleConfirm = (function ($) {
	var hashMap = {};
	...
	function Confirm (opts) {
		...
		this.opts = _.extend({
			...
			okHTML: _dms_localized.l10n.ok,
			cancelHTML: _dms_localized.l10n.cancel,
			customClass: ''
		}, opts);
	}
	...
	Confirm.prototype.show = function () {
		...
		dms.soleModal.show(this.id, this._getHtml(), {
			wrapWithTable: false,
			showCloseButton: false,
			allowClose: false, // a confirm window can't be closed on click of it's backdrop
			backdrop: this.opts.backdrop,
			customClass: 'dms-sole-confirm-modal dms-sole-confirm-' + this.opts.severity + ' ' + this.opts.customClass,
	...
	Confirm.prototype.hide = function (reason) {
		...
		dms.soleModal.hide(this.id);
	};

	//////////////////

	Confirm.prototype._fireEvents = function ($modal) {
		$modal.attr('data-dms-sole-confirm-id', this.id);

		$modal.find('.dms-sole-confirm-button')
			.add(
				$modal.find('.media-modal-backdrop')
			)
			.on('click.dms-sole-confirm', _.bind(this._handleClose, this));
	...
	Confirm.prototype._handleClose = function (event) {
		...
		} else if (! $el.hasClass('dms-sole-confirm-button')) {
			$el = $el.closest('.dms-sole-confirm-button');
		}

		var action = $el.attr('data-dms-sole-confirm-action') || 'reject';
		var id = $el.closest('.dms-sole-modal').attr('data-dms-sole-confirm-id');
		var confirm = hashMap[id];

		if (confirm) {
			var modal_container = $el.closest('.dms-sole-modal')[0];
			...
			if (action === 'reject') {
				...
				_.contains(['resolve'], action) &&
					confirm.result[action]({
						confirm: confirm,
						modal_container: $el.closest('.dms-sole-modal')[0]
					});
	...
	Confirm.prototype._getHtml = function () {
		...
		var heading = '<h1>' + dms.capitalizeFirstLetter(this.opts.severity) + '</h1>';
		...
		var cancelButton = $('<button>', {
			...
			'data-dms-sole-confirm-action': 'reject',
			type: 'button',
		}).addClass('dms-sole-confirm-button button');

		var okButton = $('<button>', {
			html: this.opts.okHTML
		}).attr({
			'data-dms-sole-confirm-action': 'resolve',
			type: 'button',
		}).addClass('dms-sole-confirm-button button button-primary');
