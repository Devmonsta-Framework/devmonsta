(function ($) {
	...
	dms.options.register('dms-undefined', {
	...
	function defaultGetValue (optionDescriptor) {
		 ...
		 */
		if (optionDescriptor.type === 'dms-undefined') {
		...
		//    name="dms_options[nesting][laptop]"
		//
		//    This step should get
		//      inputValues['dms_options']['nesting']['laptop']
		...
		if (optionDescriptor.pathToTheTopContext.length > 0) {
			var IDs = optionDescriptor.pathToTheTopContext.map(
				dms.options.getOptionDescriptor
			);
		...
		$.ajax({
			...
			data: {
				...
				name_prefix: 'dms_options',
				options: [
					options
				],
				dms_options: inputValues
			}
		})
	...
	function defaultStartListeningForChanges (optionDescriptor) {
		if (optionDescriptor.el.classList.contains('dms-listening-started')) {
			return;
		}

		optionDescriptor.el.classList.add('dms-listening-started');
		...
		if (optionDescriptor.hasNestedOptions) {
			dms.options.on.changeByContext(optionDescriptor.el, function (nestedDescriptor) {
				dms.options.trigger.changeForEl(optionDescriptor.el);
			});
		}
	}
	...
	function listenToChangesForCurrentOptionAndPreserveScoping (el, callback) {
		jQuery(el).find(
			...
			jQuery(el).find(
				'.dms-backend-option-descriptor input'
			).add(
				jQuery(el).find(
					'.dms-backend-option-descriptor select'
				)
			).add(
				jQuery(el).find(
					'.dms-backend-option-descriptor textarea'
				)
			).add(
				jQuery(el).find(
					'.dms-backend-options-virtual-context input'
				)
			).add(
				jQuery(el).find(
					'.dms-backend-options-virtual-context select'
				)
			).add(
				jQuery(el).find(
					'.dms-backend-options-virtual-context textarea'
				)
			)
		).on('change', callback);
	}
	...
	function findInputsFromAContextAndIgnoreVirtualScopes (el) {
		return jQuery(el).find(
			...
			jQuery(el).find(
				'.dms-filter-from-serialization input'
			).add(
				jQuery(el).find(
					'.dms-filter-from-serialization select'
				)
			).add(
				jQuery(el).find(
					'.dms-filter-from-serialization textarea'
				)
			)
		);
	}
